...
case class SendNotificationApiRequest(context: GoogleFCMContext, code: String, phoneNumber: String)
    extends Request[SendNotification] {
  ...
  val buildMessage: JsValue =
    Json.obj("to" -> context.to, "data" -> Json.obj("sms" -> code, "numero" -> phoneNumber))
