...
import user.{ EventStore, OrderManager, UserManager, UserView }
...
object VolskayaActorSystem {
  ...
  val userManagementActor = system.actorOf(
    Props(classOf[UserManager], Mongo.usersCollection, googleMapsContext, fcmContext),
    "userManagementActor"
  )
  val orderManagementActor =
    system.actorOf(Props(classOf[OrderManager], Mongo.ordersCollection), "orderManagementActor")
  val userView   = system.actorOf(Props[UserView], "usersViewManagementActor")
  val orderView  = system.actorOf(Props[OrderView], "orderViewManagementActor")
  val eventStore = system.actorOf(Props[EventStore], "eventManagementActor")
