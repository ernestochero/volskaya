...
case class UserManagerAPI(system: ActorSystem) {
  ...
  def updatePassword(id: String, oldPassword: String, newPassword: String): Future[UpdateResult] = {
    (userManagementActor ? UpdatePassword(new ObjectId(id), oldPassword, newPassword)).mapTo[UpdateResult]
  }
  ...
  def sendVerificationCode(verificationCode: String, phoneNumber: String): Future[SendNotification] = {
    (userManagementActor ? SendVerificationCode(verificationCode, phoneNumber)).mapTo[SendNotification]
  }
  ...
  def calculatePriceRoute(coordinateStart: Coordinate, coordinateFinish: Coordinate): Future[DistanceMatrix] = {
    (userManagementActor ? CalculatePriceRoute(coordinateStart, coordinateFinish)).mapTo[DistanceMatrix]
  }
